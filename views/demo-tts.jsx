...
export default class Demotts extends Component {
  constructor(props) {
    super(props);
    this.state = {
      voice: voices[1], // Allison v3 is the first voice
      error: null, // the error from calling /classify
      text: voices[1].demotts.text, // default text
      ssml: voices[1].demotts.ssml, // SSML text
      ssml_voice: voices[1].demotts.ssml_voice, // Voice SSML text, only some voices support this
      ssmlLabel: 'SSML',
      current_tab: 0,
      loading: false,
    };
  ...
  onResetClick() {
    ...
    this.setState({
      error: null,
      hasAudio: false,
      text: voice.demotts.text,
      ssml: voice.demotts.ssml,
      ssml_voice: voice.demotts.ssml_voice,
    });
  }
  ...
  render() {
    const {
      ssml, voice, loading, hasAudio, ssmlLabel, error, text,
    } = this.state;
      ...
      <section className="_container _container_large">
        ...
        <div className="row">
          <h1 className="base--h1 title">
            <b>AI Infinity Text Utterance</b>
          </h1>
        </div>
